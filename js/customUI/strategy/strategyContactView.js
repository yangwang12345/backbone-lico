...
define([
	...
	return Marionette.ItemView.extend({
		...
		onRender: function() {
			...
			$(".s-contact-left").on('click',function(){
				ajaxGetData("/api/alarm/email/","email");
			})
			$(".s-contact-center").on('click',function(){
				ajaxGetData("/api/alarm/sms/","sms");
			})
			$(".s-contact-right").on('click',function(){
				ajaxGetData("/api/alarm/wechat/","chat");
			})
			ajaxGetData("/api/alarm/email/",'left');
			ajaxGetData("/api/alarm/sms/",'center');
			ajaxGetData("/api/alarm/wechat/",'right');
			...
			function ajaxGetData(url, type){
				...
				function showError(){
					$('.s-contact-'+type+'-box').show();
					$('.s-contact-'+type).css("cursor","pointer");
                	$("input,select",'.s-contact-'+type+'-box').each(function (i) {
                		$(this).attr('disabled', 'disabled');
                		$(this).val('')
                	})
            		$("#"+t+"_test").attr('disabled','disabled');
            		$("#"+t+"_confirm").attr('disabled','disabled');
	                if(type=="right"){
	                	$("#qr-box").unbind("click");
            			showQRerror()
	                }
				...
				$.ajax({
					...
					success:function(data){
						if(data.ret=="success"){
	                		$("#"+t+"_test").removeAttr('disabled');
	                		$("#"+t+"_confirm").removeAttr('disabled');
							$('.s-contact-'+type+'-box').show();
							$('.s-contact-'+type).unbind('click');
							$('.s-contact-'+type).css("cursor","context-menu");
			                $("input,select", '.s-contact-'+type+'-box').each(function (i) {
			                    var $this = $(this);
			                    ...
			                    $.each(data, function (index, item) {
			                    	if ($this.attr("name") == index) {
				                        if ($this.attr("name") == "ssl") {
											...
											$("#port").html(str);
                                            $this.val(item);
                                            $("#sended").html(data.sended);
                                        } else {
				                        	$this.val(item)
				                        }
			                    	} else if (index == "enabled") {	
										setInputChecked(item, t.charAt(0)+"_status")
									}
			                    })
			                });
			                if(type=="right"){
